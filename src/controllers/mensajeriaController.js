...
controller.enviarVerificacionEmail = async (email, codigoVerificacion) => {
    try {
        await emailService.sendEmail(email, 'Código de verificación', `<p>Tu código de verificación es:</p><strong>${codigoVerificacion}</strong>`)
    } catch (error) {
    console.log(error);
